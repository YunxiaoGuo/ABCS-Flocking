...
class CS_flocking():
    ...
    def __init__(self,n_agents):
        ...
        self.state_space = MultiAgentActionSpace(
            [spaces.Box(low=np.array([-15000,-15000,-4*np.pi,-np.pi/12,
                                      -np.pi/6,self.v_min ,self.v_min ]),
                        high=np.array([15000,15000,4*np.pi,np.pi/12,
                                       np.pi/6,self.v_max,self.v_max]), shape=(7,)) for _ in range(self.n)])
        self.observation_space = MultiAgentActionSpace(
            [spaces.Box(low=np.array([-15000,-15000,-4*np.pi,-np.pi/12,
                                      -np.pi/12,self.v_min,self.v_min]),
                        high=np.array([15000,15000,4*np.pi,np.pi/12,
                                       np.pi/12,self.v_max,self.v_max]), shape=(7,)) for _ in range(self.n)])
        self.reset()
    ...
    def generator(self):
        ...
        v=[self.v_min + np.random.rand() for i in range(len(x))]
    ...
    def step(self, action: float):
        ...
        state[0][4] = np.clip(state[0][4],self.v_min,self.v_max)
        ...
        for j in range(1,len(state)):
            ...
            state[j][4] = np.clip(state[j][4],self.v_min,self.v_max)
        #update x,y,psi
        for k in range(len(state)):
            ...
            while abs(state[k][2])>2*np.pi:
                ...
                if state[k][2]<0:
                    state[k][2]+=2*np.pi
            state[k][0] += state[k][4]*np.cos(state[k][2])
            state[k][1] += state[k][4]*np.sin(state[k][2])
            
        for i in range(self.n):
            #rwd = self.Q_flocking_reward(state,i+1) # Turn on LG-CS Reward
            rwd = self.L_G_CSreward(state,i+1) # Turn on Q-flocking Reward
            #rwd = self.CSreward(state,i+1) # Turn on C-S Reward
            reward[i]+=rwd
    ...
    def CSreward(self,state, num):
        # Traditional Cucker-Smale Flocking Reward
        beta = 0.5
        sigma = 10
        reward = -np.sqrt((state[num][4]) ** 2 - 2 * np.sqrt((state[num][4]) ** 2 * (state[0][4]) ** 2) * np.cos(
            state[num][2] - state[0][2]) + (state[0][4]) ** 2) / np.sqrt(
            sigma ** 2 + (state[num][0] - state[0][0]) ** 2 + (state[num][1] - state[0][1]) ** 2) ** beta + (
                             state[num][0] - state[0][0]) ** 2 + (state[num][1] - state[0][1]) ** 2
        return reward

    def get_F(self,state_i, state_j):
        beta = 0.5
        sigma = 10
        c1 = 1
        c2 = 0
        delta_v = np.sqrt(
            (state_i[4]) ** 2 - 2 * (state_i[4]) * (state_j[4]) * np.cos(state_i[2] - state_j[2]) + (state_j[4]) ** 2)
        delta_r = np.sqrt((state_i[0] - state_j[0]) ** 2 + (state_i[1] - state_j[1]) ** 2)
        f = (delta_v + c2) / np.sqrt(sigma ** 2 + delta_r ** 2) + c1 * delta_r
        return f

    def L_G_CSreward(self,state, num):
        ...
        for i in range(0, len(state)):
            f = self.get_F(state[num], state[i])
            F.append(f)
            if i != 0:
                g = self.get_F(state[i], state[0])
            else:
                g = 0
            alpha.append(g)
